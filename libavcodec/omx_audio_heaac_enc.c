...
static int channels_to_ChannelMapping(int channels, OMX_AUDIO_CHANNELTYPE channelMapping[OMX_AUDIO_MAXCHANNELS])
{
    OMX_AUDIO_CHANNELTYPE res[][8] = {
        ...
        { OMX_AUDIO_ChannelLF, OMX_AUDIO_ChannelRF, OMX_AUDIO_ChannelCF, OMX_AUDIO_ChannelLS, OMX_AUDIO_ChannelRS },// TODO: OMX_AUDIO_ChannelLFE ?
        { OMX_AUDIO_ChannelLF, OMX_AUDIO_ChannelRF, OMX_AUDIO_ChannelCF, OMX_AUDIO_ChannelLS, OMX_AUDIO_ChannelRS },
        { OMX_AUDIO_ChannelLF, OMX_AUDIO_ChannelRF, OMX_AUDIO_ChannelCF, OMX_AUDIO_ChannelLS, OMX_AUDIO_ChannelRS, OMX_AUDIO_ChannelLR, OMX_AUDIO_ChannelRR },// TODO: OMX_AUDIO_ChannelLFE ?
...
static int sample_fmt_to_omx_index(OMX_AUDIO_PARAM_PCMMODETYPE *audio_param, AVCodecContext *avctx, enum AVSampleFormat sample_fmt)  // TODO : move it to common code
{
...
static int omx_set_audio_heaac_param(AVCodecContext *avctx)
{
    ...
    audio_param_aac.eAACStreamFormat = avctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER ? OMX_AUDIO_AACStreamFormatRAW : OMX_AUDIO_AACStreamFormatMP4LATM; // TODO (it also could be set via heaac library commandline parser)
    audio_param_aac.nAudioBandWidth = 0;
    audio_param_aac.nFrameLength = 0;
    audio_param_aac.nAACtools = 0;
    audio_param_aac.nAACERtools = 0;
    audio_param_aac.eChannelMode = OMX_AUDIO_ChannelModeVendorStartUnused; // TODO
...
static av_cold int omx_cmpnt_encoder_init(AVCodecContext *avctx)
{
    ...
    avctx->frame_size = audio_param_xheaac->nFrameSamples; //avctx->sample_rate / (10 * avctx->channels);
